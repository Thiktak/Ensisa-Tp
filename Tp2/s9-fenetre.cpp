...
#define WIDTH  600
#define HEIGHT 600
#define TITLE  "GLUT Demo: Utilisation des sous-fenetres"
int winIdMain;
int winIdSub;
...
mainDisplay (void)
{
  ...
  sprintf (label, "(Test des sous-fen�etres %3.5f)",spin);
...
subDisplay ()
{
  ...
  glColor3f (1.0F, 1.0F, 1.0F);
  sprintf (label, "Temps = %8.3f ", time);
  glRasterPos2f (0.05F, 0.75F);
  drawString (label);

  sprintf (label, "Angle de rotation  = %8.3f ", spin);
  glRasterPos2f (0.05F, 0.55F);
  drawString (label);

  sprintf (label, "Vitesse de rotation= %8.3f ", spin / time);
  glRasterPos2f (0.05F, 0.35F);
  drawString (label);

  /* Type Banner and info */
  glColor3f (1.0F, 0.0F, 1.0F);
  sprintf (label, " C'est une sous-fenetre ");
  glRasterPos2f (0.40F, 0.70F);
  drawStringBig (label);

  sprintf (label, " Elle a son propre context OpenGL ");
